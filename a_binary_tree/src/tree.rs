    ...
    V: Clone,
{
    fn determine_deletion_type(&self) -> DeletionType {
        let childrenvec: Vec<Option<Box<Node<K, V>>>> =
            vec![self.left_child.clone(), self.right_child.clone()];
    ...
    V: Clone + Debug,
{
    ...
    fn find_internal(&self, root: Option<Box<Node<K, V>>>, key: &K) -> Option<V> {
        if root.is_none() {
            ...
            match boxclone.key.cmp(&key) {
                Ordering::Less => self.find_internal(root.unwrap().left_child, key),
                Ordering::Equal => Some(root.unwrap().value),
                Ordering::Greater => self.find_internal(root.unwrap().right_child, key),
            }
        }
    }

    pub fn insert(&mut self, key: K, value: V) {
        BinaryTree::insert_internal(&mut self.root, &mut NodeStatus::IsRoot, key, value);
    }
    fn insert_internal(
        ...
        let mutnode = root.as_mut().unwrap().as_mut();
        match mutnode.key.cmp(&key) {
            Ordering::Less => {
                BinaryTree::insert_internal(
                    &mut mutnode.left_child,
                    &mut NodeStatus::IsLeftChild,
                    key,
                    value,
                );
            }
            ...
            Ordering::Greater => {
                BinaryTree::insert_internal(
                    &mut mutnode.right_child,
                    &mut NodeStatus::IsRightChild,
                    key,
                    value,
                );
            }
    ...
    fn delete_internal(
        &mut self,
        root: &mut Option<Box<Node<K, V>>>,
        parent: &mut Option<Box<Node<K, V>>>,
        key: &K,
    ) -> Result<(), ()> {
        if root.is_none() {
            return Err(());
    ...
    fn perform_deletion(
        ...
        match delete_me_node.determine_deletion_type() {
            ...
            DeletionType::HasLeftChild => match delete_me_node.status {
                NodeStatus::IsLeftChild => {
                    let mut parent_box = parent.clone().unwrap();
                    let parent_node = parent_box.as_mut();
            ...
            DeletionType::HasRightChild => match delete_me_node.status {
                NodeStatus::IsLeftChild => {
                    let mut parent_box = parent.clone().unwrap();
                    let parent_node = parent_box.as_mut();
            ...
            DeletionType::HasBothChildren => match delete_me_node.status {
                NodeStatus::IsLeftChild => {
                    let mut parent_box = parent.clone().unwrap();
                    let parent_node = parent_box.as_mut();
                    let delete_me_left_child = delete_me.clone().unwrap().left_child;
